...
const (
	...
	logfileDir     = "logfiles"
	linuxPuppetCmd = "puppet agent"
	linuxConfDir   = "/etc/puppetlabs"
)
...
type Provisioner struct {
	...
	installPuppetAgent func(terraform.UIOutput, communicator.Communicator) error
	createConfigFiles  func(terraform.UIOutput, communicator.Communicator) error
	runPuppetAgent     func(terraform.UIOutput, communicator.Communicator) error
	useSudo            bool
...
func (r *ResourceProvisioner) Apply(
	...
	case "linux":
		...
		p.runPuppetAgent = p.runPuppetAgentFunc(linuxPuppetCmd, linuxConfDir)
	...
	if !p.SkipInstall {
		if err := p.installPuppetAgent(o, comm); err != nil {
			return err
		}
	}

	o.Output("Starting initial puppet-agent run...")
	if err := p.runPuppetAgent(o, comm); err != nil {
		return err
	}
...
func (p *Provisioner) runPuppetAgentFunc(
	puppetCmd string,
	...
	return func(o terraform.UIOutput, comm communicator.Communicator) error {
		...
		if p.UsePolicyfile {
			cmd = fmt.Sprintf("%s -j %q", puppetCmd, fb)
		} else {
			cmd = fmt.Sprintf("%s -j %q -E %q", puppetCmd, fb, p.Environment)
		}
		...
		if p.LogToFile {
			...
			o.Output("Writing puppet-agent output to " + logFile)
			o = p
		}
