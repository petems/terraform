...
const (
	...
	installURL = "https://raw.githubusercontent.com/petems/puppet-install-shell/master/install_puppet_agent.sh"
)

func (p *Provisioner) linuxInstallPuppetAgent(
	...
	err = p.runCommand(o, comm, fmt.Sprintf("%sbash ./install_puppet_agent.sh -v %q", prefix, p.Version))
	if err != nil {
		return err
	}

	// And finally cleanup the install.sh script again
	return p.runCommand(o, comm, fmt.Sprintf("%srm -f install_puppet_agent.sh", prefix))
}
